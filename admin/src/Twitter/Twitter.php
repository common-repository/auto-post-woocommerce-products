...
class Twitter {
	...
	public function __construct(
		...
		$this->consumer = new Twitter_OAuthConsumer( $consumerKey,
			$consumerSecret );
		$this->token = new Twitter_OAuthConsumer( $accessToken,
			$accessTokenSecret );
	}
	...
	public function authenticate() {
		try {
			$res = $this->request( 'account/verify_credentials',
				'GET' );
	...
	/**
	 * Retweets a tweet from a specified search by CIL Creations
	 * @param    string $id      The id# of the post to retweet
	 * @param    string $message The retweet @
	 * @return
	 */
	...
	public function follow( $username ) {
		return $this->request( 'friendships/create',
			'POST',
			[
				'screen_name' => $username] );
	}
	...
	public function load(
		...
		return $this->cachedRequest( 'statuses/' . $timelines[$flags & 3],
			(array) $data + [
				'count'       => $count,
	...
	public function loadUserInfo( $username ) {
		return $this->cachedRequest( 'users/show',
			[
				'screen_name' => $username] );
	}
	...
	public function search(
		...
		$res = $this->request( 'search/tweets',
			'GET',
	...
	public function request(
		...
		$hasCURLFile = class_exists( 'CURLFile',
			false ) && defined( 'CURLOPT_SAFE_UPLOAD' );

		foreach ( (array) $data as $key => $val ) {
			if ( null === $val ) {
				unset( $data[$key] );
			} elseif ( $files && ! $hasCURLFile && substr( $val,
				0,
				1 ) === '@' ) {
		...
		$request = Twitter_OAuthRequest::from_consumer_and_token( $this->consumer,
			$this->token,
			$method,
			$resource,
			$files ? [
			] : $data );
		$request->sign_request( new Twitter_OAuthSignatureMethod_HMAC_SHA1,
			$this->consumer,
			$this->token );
		...
		$payload = defined( 'JSON_BIGINT_AS_STRING' ) ? @json_decode( $result,
			false,
			128,
			JSON_BIGINT_AS_STRING ) : @json_decode( $result ); // intentionally @
		...
		$code = curl_getinfo( $curl,
			CURLINFO_HTTP_CODE );
		if ( $code >= 400 && 403 != $code ) {
			throw new TwitterException( isset( $payload->errors[0]->message ) ? $payload->errors[0]->message : "Server error #$code with answer $result",
				$code
			);
		}
	}
	...
	public function cachedRequest(
		...
		. '/twitter.'
		. md5( $resource . json_encode( $data ) . serialize( [
			$this->consumer,
			$this->token] ) )
			. '.json';
	...
	public static function clickable( stdClass $status ) {
		...
		foreach ( $status->entities->hashtags as $item ) {
			$all[$item->indices[0]] = [
				"https://twitter.com/search?q=%23$item->text",
				"#$item->text",
				$item->indices[1]];
		}
		...
		foreach ( $status->entities->user_mentions as $item ) {
			$all[$item->indices[0]] = [
				"https://twitter.com/$item->screen_name",
				"@$item->screen_name",
				$item->indices[1]];
		}
